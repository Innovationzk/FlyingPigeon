...
* 消息体
```C++
#define EV_UI_CLT_REGIST_CMD               (u16)18      // UI通知底层注册用户名
typedef struct {
	s8 userName[FILE_BUF_SIZE];     // 用户名
}ui_clt_regist_cmd;

#define EV_UI_CLT_POST_FILE_CMD            (u16)19      // UI通知底层开始发送文件
typedef struct {
	u8 fileNum;                     // 文件数量
	s8 fileName[FILE_BUF_SIZE];     // 文件名
}ui_clt_post_file_cmd;

#define EV_UI_CLT_POST_MSG_CMD             (u16)20      // UI通知底层开始发送消息
typedef struct {
	u8 clientNum;                   // 目标客户端数量
	u16 clientAppID[16];            // 目标客户端节点号
	s8 msgContent[FILE_BUF_SIZE];   // 消息内容
}ui_clt_post_msg_cmd;

#define EV_CLT_UI_POST_FILE_NACK           (u16)21      // 底层通知UI服务器繁忙，拒绝发送文件

#define EV_CLT_UI_POST_FILE_PROGRESS_NTF   (u16)22      // 底层通知UI文件发送进度
typedef struct {
	u8 fileNo;                     // 文件编号
	s32 fileProgress;              // 发送进度
}clt_ui_post_file_progress_ntf;

#define EV_CLT_UI_POST_FILE_COMPLETE_NTF   (u16)23      // 底层通知UI文件发送完成
typedef struct {
	u8 fileNo;                     // 文件编号
}clt_ui_post_file_complete_ntf;

#define EV_CLT_UI_POST_FILE_ERROR_NTF      (u16)24      // 底层通知UI文件校验失败，需重新发送
typedef struct {
	u8 fileNo;                     // 文件编号
}clt_ui_post_file_error_ntf;

#define EV_CLT_UI_POST_MSG_COMPLETE_NTF    (u16)25      // 底层通知UI消息发送成功
#define EV_CLT_UI_POST_MSG_ERROR_NTF       (u16)26      // 底层通知UI服务器资源不足，消息发送失败

#define EV_CLT_UI_OCCUPIED_NAE_NACK        (u16)27      // 底层通知UI名称被占用，拒绝注册
#define EV_CLT_UI_NO_RESOURCES_NACK        (u16)28      // 底层通知UI服务端资源不足
#define EV_CLT_UI_CLIENT_LIST_NTF          (u16)29      // 底层通知UI在线客户端列表 
typedef struct {
...
#define EV_CLT_UI_RECEIVE_MSG_NTF          (u16)30      // 底层通知UI收到消息
typedef struct {
	u32 srcNode;                    // 源节点
	s8 msgContent[FILE_BUF_SIZE];   // 消息内容
}clt_ui_receive_msg_ntf;

#define EV_CLT_UI_SERVER_DISCONNECT_NTF    (u16)31      // 底层通知UI服务器断链
typedef struct {
	u8 fileNum;                     // 文件数量
...
#define EV_CLT_UI_POST_FILE_RECONNECT_NTF  (u16)32      // 重新连接后通知UI继续发送文件
typedef struct {
	u8 fileNum;                     // 文件数量
	s32 fileProgress[MAX_FILE_NUM]; // 文件传输进度
	s8 fileName[FILE_BUF_SIZE];     // 文件名
...
```mermaid
sequenceDiagram
    Client->>Client:记录当前文件名，编号，文件大小
    loop transfer
        Client ->> Server: 发送文件
        note over Server:记录发送进度
    end
    Client -->> Server: 断开链接
    Client->>Client: 关闭当前任务
